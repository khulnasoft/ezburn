...
import (
	...
	"github.com/khulnasoft/ezburn/internal/api_helpers"
...
` + colors.Bold + `Documentation:` + colors.Reset + `
  ` + colors.Underline + `https://ezburn.github.io/` + colors.Reset + `

` + colors.Bold + `Repository:` + colors.Reset + `
  ` + colors.Underline + `https://github.com/khulnasoft/ezburn` + colors.Reset + `
...
` + colors.Bold + `Advanced options:` + colors.Reset + `
  ...
  --metafile=...            Write metadata about the build to a JSON file
                            (see also: ` + colors.Underline + `https://ezburn.github.io/analyze/` + colors.Reset + `)
...
func main() {
	...
	for _, arg := range osArgs {
		...
		case strings.HasPrefix(arg, "--timing"):
			// This is a hidden flag because it's only intended for debugging ezburn
		...
		default:
			// Some people want to be able to run ezburn's watch mode such that it
			// never exits. However, ezburn ends watch mode when stdin is closed
			...
			// accidentally creating ezburn processes that live forever.
			//
	...
	func() {
		...
		if cpuprofileFile != "" {
			...
			// return useful information for ezburn, since it's so fast. Let's keep
			...
			if !isServeOrWatch {
				...
				// since otherwise ezburn could unnecessarily use much more memory
				...
				// This is necessary when the ezburn binary executable is invoked via
				...
				go func() {
					...
					for {
						...
						// Some people attempt to keep ezburn's watch mode open by piping
						// an infinite stream of data to stdin such as with "< /dev/zero".
						// This will make ezburn spin at 100% CPU. To avoid this, put a
