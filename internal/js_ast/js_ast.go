...
type Class struct {
	...
	// because TypeScript (and ezburn) transforms the above class into this:
	//
	//   class Foo {
	//     static set foo(x) { importantSideEffect(x); }
	//   }
...
type EUnary struct {
	...
	// checks are not yet handled correctly by ezburn, so this possibility is
	// currently ignored.
...
type EJSXElement struct {
	...
	// ezburn's output. These absent AST nodes are completely omitted when
...
// https://github.com/khulnasoft/ezburn/issues/3605
