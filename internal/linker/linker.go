...
import (
	...
	"github.com/khulnasoft/ezburn/internal/css_ast"
	"github.com/khulnasoft/ezburn/internal/css_lexer"
	"github.com/khulnasoft/ezburn/internal/css_parser"
	"github.com/khulnasoft/ezburn/internal/css_printer"
	"github.com/khulnasoft/ezburn/internal/fs"
	"github.com/khulnasoft/ezburn/internal/graph"
	...
	"github.com/khulnasoft/ezburn/internal/js_printer"
	"github.com/khulnasoft/ezburn/internal/logger"
	"github.com/khulnasoft/ezburn/internal/renamer"
	"github.com/khulnasoft/ezburn/internal/resolver"
	"github.com/khulnasoft/ezburn/internal/runtime"
	"github.com/khulnasoft/ezburn/internal/sourcemap"
	"github.com/khulnasoft/ezburn/internal/xxhash"
)
			...
			for _, importRecordIndex := range part.ImportRecordIndices {
				...
				if otherRepr.Meta.Wrap != graph.WrapNone {
					...
					if otherRepr.Meta.Wrap == graph.WrapESM && record.Kind != ast.ImportStmt {
						...
						// https://github.com/khulnasoft/ezburn/issues/1591.
						if record.Kind == ast.ImportRequire {
...
func (c *linkerContext) generateEntryPointTailJS(
	...
	case config.FormatCommonJS:
		...
		if c.options.Platform == config.PlatformNode {
			...
			//
			//   import { name } from './file-from-ezburn.cjs'
			//
			// when "file-from-ezburn.cjs" looks like this:
			//
			//   __export(exports, { name: () => name });
			//   0 && (module.exports = {name});
			//
			// The maintainer of "cjs-module-lexer" is receptive to adding ezburn-
...
func (c *linkerContext) renameSymbolsInChunk(chunk *chunkInfo, filesInOrder []uint32, timer *helpers.Timer) renamer.Renamer {
	...
	// this happened: https://github.com/khulnasoft/ezburn/issues/3544
	...
	if chunkRepr, ok := chunk.chunkRepr.(*chunkReprJS); ok {
		for _, partRange := range chunkRepr.partsInChunkInOrder {
			...
			if file.InputFile.Source.KeyPath.Namespace == "file" {
				...
				// that problem isn't caused by ezburn itself.
