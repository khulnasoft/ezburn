...
import (
	...
	"github.com/khulnasoft/ezburn/internal/ast"
	"github.com/khulnasoft/ezburn/internal/compat"
	"github.com/khulnasoft/ezburn/internal/config"
	"github.com/khulnasoft/ezburn/internal/helpers"
	"github.com/khulnasoft/ezburn/internal/js_ast"
	"github.com/khulnasoft/ezburn/internal/js_lexer"
	"github.com/khulnasoft/ezburn/internal/logger"
	"github.com/khulnasoft/ezburn/internal/renamer"
...
type parser struct {
	...
	// This has caught real issues in the past. However, it's not ezburn's job
	// to find bugs in other libraries, and these warnings are problematic for
	// people using these libraries with ezburn. The only fix is to either
	// disable all ezburn warnings and not get warnings about your own code, or
	...
	//   })(ns || (ns = {}));
	//
	// Relevant issue: https://github.com/khulnasoft/ezburn/issues/1158
...
func (p *parser) popScope() {
	...
	if p.currentScope.ContainsDirectEval {
		for _, member := range p.currentScope.Members {
			...
			// ezburn must assume that it can potentially reach anything in any of
				...
				case "declare":
					if !p.lexer.HasNewlineBefore && opts.isClass && p.options.ts.Parse && opts.tsDeclareRange.Len == 0 {
						...
						if prop, ok := p.parseProperty(startLoc, kind, opts, nil); ok &&
							...
							// References:
							//
							//   https://github.com/khulnasoft/ezburn/issues/1675
				...
				case "abstract":
					if !p.lexer.HasNewlineBefore && opts.isClass && p.options.ts.Parse && !opts.isTSAbstract {
						...
						if prop, ok := p.parseProperty(startLoc, kind, opts, nil); ok &&
							...
							// References:
							//
							//   https://github.com/khulnasoft/ezburn/issues/3684
							//
							prop.Kind = js_ast.PropertyDeclareOrAbstract
							return prop, true
						}
	...
	case js_lexer.TLessThan:
		...
		if !p.options.jsx.Parse && !p.options.ts.Parse {
			...
			p.log.AddErrorWithNotes(&p.tracker, p.lexer.Range(), "The JSX syntax extension is not currently enabled", []logger.MsgData{{
				Text: "The ezburn loader for this file is currently set to \"js\" but it must be set to \"jsx\" to be able to parse JSX syntax." + how}})
			...
			if expr, ok := stmt.Data.(*js_ast.SExpr); ok {
				if str, ok := expr.Value.Data.(*js_ast.EString); ok && !str.PreferTemplate {
					...
					if helpers.UTF16EqualsString(str.Value, "use strict") {
						...
						// mind and round-tripping asm.js code through ezburn will very
					...
					if str, ok := property.Key.Data.(*js_ast.EString); ok {
						...
						case "__source", "__self":
							...
							p.log.AddErrorWithNotes(&p.tracker, r,
								...
								[]logger.MsgData{{Text: "Both \"__source\" and \"__self\" are set automatically by ezburn when using React's \"automatic\" JSX transform. " +
		...
		default:
			...
			case js_ast.UnOpVoid:
				...
				if p.options.minifySyntax {
					...
					// running ezburn on weird kinda-JavaScript-but-not languages and
					...
					//
					// More info: https://github.com/khulnasoft/ezburn/issues/4041
			...
			if e.OptionalChain == js_ast.OptionalChainNone {
				...
				if wasIdentifierBeforeVisit && symbol.OriginalName == "eval" {
					...
					p.log.AddIDWithNotes(logger.MsgID_JS_DirectEval, kind, &p.tracker, js_lexer.RangeOfIdentifier(p.source, e.Target.Loc), text,
						[]logger.MsgData{{Text: "You can read more about direct eval and bundling here: https://ezburn.github.io/link/direct-eval"}})
				...
				if p.options.mode != config.ModePassThrough {
					...
					if len(e.Args) == 1 {
						...
						p.log.AddIDWithNotes(logger.MsgID_JS_UnsupportedRequireCall, logger.Debug, &p.tracker, r,
							...
							[]logger.MsgData{{Text: "To be bundled by ezburn, a \"require\" call must have exactly 1 argument."}})
					}
...
// only really relevant for ezburn's WebAssembly-based API.
				...
				if dot, ok := e.Left.Data.(*js_ast.EDot); ok {
					...
					if name != "" {
						...
						if why == whyESMTypeModulePackageJSON {
							...
							if p.options.ts.Parse {
								text += " If you are using TypeScript, you can use the \".cts\" file extension with ezburn instead."
							}
...
func (p *parser) scanForImportsAndExports(stmts []js_ast.Stmt) (result importsExportsScanResult) {
	...
	for _, stmt := range stmts {
		...
		case *js_ast.SImport:
			...
			// imports in ways that are impossible for TypeScript and ezburn to know
			...
			//   </main>
			//
			// Tools that use ezburn to compile TypeScript code inside a Svelte
			// file like this only give ezburn the contents of the <script> tag.
			...
			//
			//   * If identifier minification is enabled, then using ezburn as a
...
func newParser(log logger.Log, source logger.Source, lexer js_lexer.Lexer, options *Options) *parser {
	...
	p := &parser{
		...
		// running ezburn (or other tools like Terser) again on the output.
