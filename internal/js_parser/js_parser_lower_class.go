...
func (ctx *lowerClassContext) finishAndGenerateCode(p *parser, result visitClassResult) ([]js_ast.Stmt, js_ast.Expr) {
	...
	//   Foo = wrap(Foo);
	//
	// One reason to do this is that ezburn's bundler sometimes needs to lazily-
	...
	// avoid a performance hit for static "import" statements, ezburn stores
	...
	//
	// Due to ezburn's parallel architecture, we both a) need to transform class
