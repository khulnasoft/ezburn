...
export interface StreamIn {
  ...
  isSync: boolean
  hasFS: boolean
  ezburn: types.PluginBuild['ezburn']
}
...
export function createChannel(streamIn: StreamIn): StreamOut {
    ...
    try {
      ...
      if (typeof request.key === 'number') {
        ...
        if (!requestCallbacks) {
          ...
          // is called while ezburn is processing many files in parallel.
          // See https://github.com/khulnasoft/ezburn/issues/3318 for details.
          return
        }
      ...
      try {
        ...
        // This may fail if the ezburn process is no longer running, but
  ...
  let handleIncomingPacket = (bytes: Uint8Array): void => {
    ...
    if (isFirstPacket) {
      ...
      // Validate the binary's version number to make sure ezburn was installed
      ...
      if (binaryVersion !== EZBURN_VERSION) {
        throw new Error(`Cannot start service: Host version "${EZBURN_VERSION}" does not match binary version ${quote(binaryVersion)}`)
      }
      return
    }
...
function failureErrorWithLog(text: string, errors: types.Message[], warnings: types.Message[]): types.BuildFailure {
  ...
  // printed. The error objects are pretty big and not helpful because a) ezburn
...
function sanitizeLocation(location: types.PartialMessage['location'], where: string, terminalWidth: number | undefined): types.Message['location'] {
  ...
  // to avoid the performance hit. See: https://github.com/khulnasoft/ezburn/issues/3467
  if (lineText) {
...
function convertOutputFiles({ path, contents, hash }: protocol.BuildOutputFile): types.OutputFile {
  ...
  return {
    ...
    hash,
    get text() {
      // People want to be able to set "contents" and have ezburn automatically
