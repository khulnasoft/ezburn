...
let ensureServiceIsRunning = (): Promise<Service> => {
  return initializePromise || startRunningService('ezburn.wasm', undefined, true)
}
...
export const initialize: typeof types.initialize = async (options) => {
  ...
  initializePromise = startRunningService(wasmURL || 'ezburn.wasm', wasmModule, useWorker)
