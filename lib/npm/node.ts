...
import { EZBURN_BINARY_PATH, generateBinPath } from "./node-platform"
...
// This file is used for both the "ezburn" package and the "ezburn-wasm"
// package. "WASM" will be true for "ezburn-wasm" and false for "ezburn".
declare const WASM: boolean

let worker_threads: typeof import('worker_threads') | undefined

if (process.env.EZBURN_WORKER_THREADS !== '0') {
...
let isInternalWorkerThread = worker_threads?.workerData?.ezburnVersion === EZBURN_VERSION

let ezburnCommandAndArgs = (): [string, string[]] => {
  // Try to have a nice error message when people accidentally bundle ezburn
  // without providing an explicit path to the binary, or when using WebAssembly.
  if ((!EZBURN_BINARY_PATH || WASM) && (path.basename(__filename) !== 'main.js' || path.basename(__dirname) !== 'lib')) {
    throw new Error(
      `The ezburn JavaScript API cannot be bundled. Please mark the "ezburn" ` +
      `package as external so it's not included in the bundle.\n` +
      `\n` +
      `More information: The file containing the code for ezburn's JavaScript ` +
      `API (${__filename}) does not appear to be inside the ezburn package on ` +
      `the file system, which usually means that the ezburn package was bundled ` +
      `into another file. This is problematic because the API needs to run a ` +
      `binary executable inside the ezburn package which is located using a ` +
      `relative path from the API code to the executable. If the ezburn package ` +
      `is bundled, the relative path will be incorrect and the executable won't ` +
      `be found.`)
  }

  if (WASM) {
    return ['node', [path.join(__dirname, '..', 'bin', 'ezburn')]]
  } else {
...
let ensureServiceIsRunning = (): Service => {
  ...
  let [command, args] = ezburnCommandAndArgs()
  let child = child_process.spawn(command, args.concat(`--service=${EZBURN_VERSION}`, '--ping'), {
    windowsHide: true,
...
let runServiceSync = (callback: (service: common.StreamService) => void): void => {
  ...
  callback(service)
  let stdout = child_process.execFileSync(command, args.concat(`--service=${EZBURN_VERSION}`), {
    cwd: defaultWD,
    windowsHide: true,
    input: stdin,
    ...
    // variable.
    maxBuffer: +process.env.EZBURN_MAX_BUFFER! || 16 * 1024 * 1024,
  })
...
let randomFileName = () => {
  return path.join(os.tmpdir(), `ezburn-${crypto.randomBytes(32).toString('hex')}`)
}
...
let startWorkerThreadService = (worker_threads: typeof import('worker_threads')): WorkerThreadService => {
  ...
  let worker = new worker_threads.Worker(__filename, {
    workerData: { workerPort, defaultWD, ezburnVersion: EZBURN_VERSION },
...
let startSyncServiceWorker = () => {
  ...
  try {
    ...
    // the time the "ezburn" package was first imported.
