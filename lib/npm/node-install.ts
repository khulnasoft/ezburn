import { downloadedBinPath, EZBURN_BINARY_PATH, isValidBinaryPath, pkgAndSubpathForCurrentPlatform } from './node-platform'
...
const toPath = path.join(__dirname, 'bin', 'ezburn')
let isToPathJS = true
...
function validateBinaryVersion(...command: string[]): void {
  ...
  try {
    stdout = child_process.execFileSync(command.shift()!, command, {
      ...
      // - https://github.com/khulnasoft/ezburn/issues/1711#issuecomment-1027554035
      //
      stdio: 'pipe',
    }).toString().trim()
  } catch (err) {
    if (os.platform() === 'darwin' && /_SecTrustEvaluateWithError/.test(err + '')) {
      ...
      throw new Error(`The "ezburn" package cannot be installed because ${os} is too outdated.

The Go compiler (which ezburn relies on) no longer supports ${os},
which means the "ezburn" binary executable can't be run. You can either:

  * Update your version of macOS to one that the Go compiler supports
  * Use the "ezburn-wasm" package instead of the "ezburn" package
  * Build ezburn yourself using an older version of the Go compiler
`)
    }
    throw err
  }
...
function installUsingNPM(pkg: string, subpath: string, binPath: string): void {
  // Erase "npm_config_global" so that "npm install --global ezburn" works.
  ...
  // Create a temporary directory inside the "ezburn" package with an empty
  // "package.json" file. We'll use this to run "npm install" in.
  const ezburnLibDir = path.dirname(require.resolve('ezburn'))
  const installDir = path.join(ezburnLibDir, 'npm-install')
...
function applyManualBinaryPathOverride(overridePath: string): void {
  // Patch the CLI use case (the "ezburn" command)
  const pathString = JSON.stringify(overridePath)
  fs.writeFileSync(toPath, `#!/usr/bin/env node\n` +
    `require('child_process').execFileSync(${pathString}, process.argv.slice(2), { stdio: 'inherit' });\n`)

  // Patch the JS API use case (the "require('ezburn')" workflow)
  const libMain = path.join(__dirname, 'lib', 'main.js')
  const code = fs.readFileSync(libMain, 'utf8')
  fs.writeFileSync(libMain, `var EZBURN_BINARY_PATH = ${pathString};\n${code}`)
}

function maybeOptimizePackage(binPath: string): void {
  // This package contains a "bin/ezburn" JavaScript file that finds and runs
  // the appropriate binary executable. However, this means that running the
  // "ezburn" command runs another instance of "node" which is way slower than
  ...
  // because on Windows the binary executable must be called "ezburn.exe"
  // instead of "ezburn".
  //
  ...
  if (os.platform() !== 'win32' && !isYarn()) {
    const tempPath = path.join(__dirname, 'bin-ezburn')
    try {
...
async function downloadDirectlyFromNPM(pkg: string, subpath: string, binPath: string): Promise<void> {
  ...
  const url = `https://registry.npmjs.org/${pkg}/-/${pkg.replace('@ezburn/', '')}-${versionFromPackageJSON}.tgz`
  console.error(`[ezburn] Trying to download ${JSON.stringify(url)}`)
  try {
    fs.writeFileSync(binPath, extractFileFromTarGzip(await fetch(url), subpath))
    fs.chmodSync(binPath, 0o755)
  } catch (e: any) {
    console.error(`[ezburn] Failed to download ${JSON.stringify(url)}: ${e && e.message || e}`)
    throw e
  }
}
...
async function checkAndPreparePackage(): Promise<void> {
  ...
  if (isValidBinaryPath(EZBURN_BINARY_PATH)) {
    if (!fs.existsSync(EZBURN_BINARY_PATH)) {
      ...
      applyManualBinaryPathOverride(EZBURN_BINARY_PATH)
      return
    }
  }
  ...
  try {
    ...
    console.error(`[ezburn] Failed to find package "${pkg}" on the file system

This can happen if you use the "--no-optional" flag. The "optionalDependencies"
package.json feature is used by ezburn to install the correct binary executable
for your current platform. This install script will now attempt to work around
this. If that fails, you need to remove the "--no-optional" flag to use ezburn.
`)
    ...
    // to be helpful but it's not the supported way of installing ezburn.
    binPath = downloadedBinPath(pkg, subpath)
    try {
      console.error(`[ezburn] Trying to install package "${pkg}" using npm`)
      installUsingNPM(pkg, subpath, binPath)
    } catch (e2: any) {
      console.error(`[ezburn] Failed to install package "${pkg}" using npm: ${e2 && e2.message || e2}`)
