...
const { installForTests } = require('./ezburn')
...
const skipTheseTests = new Set([
  // Skip these tests because ezburn deliberately always supports ESM syntax
  // in all files. Also ezburn doesn't support the script goal at all.
...
async function checkTransformAPI({ ezburn, file, content, yaml }) {
  if (yaml.flags) {
  ...
  let result
  try {
    result = await ezburn.transform(content, { sourcefile: file })
  } catch (error) {
    if (shouldParse) {
  ...
  let result2
  try {
    result2 = await ezburn.transform(result.code, { sourcefile: file })
  } catch (error) {
    error.kind = 'Reparse'
    throw error
  }
  ...
  let result4
  try {
    result4 = await ezburn.transform(result2.code, { sourcefile: file, minify: true })
  } catch (error) {
    error.kind = 'Minify'
    throw error
  }
  ...
  let result5
  try {
    result5 = await ezburn.transform(result4.code, { sourcefile: file, minify: true })
  } catch (error) {
    error.kind = 'Minify'
    throw error
  }
...
async function checkBuildAPI({ ezburn, file, content, yaml }) {
  const plugins = []
  if (yaml.flags) {
  ...
  try {
    ...
    result = await ezburn.build(options)
  } catch (error) {
    if (shouldParse) {
      error.kind = 'Build'
      throw error
  ...
  // if it still works after running ezburn's syntax lowering pass on it
  for (let version = 2015; version <= 2022; version++) {
    let result
    try {
      result = await ezburn.transform(code, { sourcefile: file, target: `es${version}` })
    } catch (error) {
      continue // This means ezburn doesn't support lowering this code to this old a version
    }
    try {
      await runCodeInHarness(yaml, code, importDir)
    } catch (error) {
...
async function main() {
  ...
  console.log(`\n${dimColor}Installing ezburn...${resetColor}`)
  ...
  await forEachInParallel(files, 32, async (file) => {
    // Don't parse files that ezburn deliberately handles differently
    ...
    try {
      ...
      await checkTransformAPI({ ezburn, file, content, yaml })
      await checkBuildAPI({ ezburn, file, content, yaml })
    }
