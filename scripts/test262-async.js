...
const ezburn = require('./ezburn').installForTests()
...
async function main() {
  ...
  for (const test of tests) {
    ...
    // Check "ezburn"
    await check('target=esnext', ezburn.transform(js, { keepNames: true, target: 'esnext' }))
    await check('target=es2017', ezburn.transform(js, { keepNames: true, target: 'es2017' }))
    await check('for-await=false', ezburn.transform(js, { keepNames: true, supported: { 'for-await': false } }))
    if (!hasYieldKeyword) {
      await check('target=es2016', ezburn.transform(js, { keepNames: true, target: 'es2016' }))
      await check('async-await=false', ezburn.transform(js, { keepNames: true, supported: { 'async-await': false } }))
      await check('for-await=false,async-await=false', ezburn.transform(js, { keepNames: true, supported: { 'for-await': false, 'async-await': false } }))
    }
