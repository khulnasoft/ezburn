...
const { buildBinary, removeRecursiveSync } = require('./ezburn')
...
async function check(kind, testCase, toSearch, { outfile, flags, entryPoints, crlf, followUpFlags = [], checkFirstChunk }) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      const child = childProcess.spawn(ezburnPath, args, { cwd: tempDir, stdio: ['pipe', 'pipe', 'inherit'] })
    ...
    for (let order of [0, 1, 2]) {
      ...
      await execFileAsync(ezburnPath, [
        nestedEntry,
        '--bundle',
