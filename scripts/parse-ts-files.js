...
// ezburn. This is useful to check for parser bugs and/or crashes in ezburn.
...
const ezburnPath = path.join(path.dirname(__dirname), 'ezburn');
...
if (process.argv.includes('--individual')) {
  walkDir(process.cwd(), absolute => {
    let output = child_process.spawnSync(ezburnPath, [absolute, '--outfile=/dev/null'], { stdio: ['inherit', 'pipe', 'pipe'] });
    if (output.status) {
      let result;
      try {
      ...
      console.log('-'.repeat(20) + ' ezburn output:');
...
// Otherwise it's much faster to do everything at once
  ...
  const tempDir = path.join(os.tmpdir(), 'ezburn-parse-ts-files');
  try {
  ...
  try {
    child_process.execFileSync(ezburnPath, ['--outdir=' + tempDir].concat(all), { stdio: 'inherit' });
  } catch (e) {
  }
}
